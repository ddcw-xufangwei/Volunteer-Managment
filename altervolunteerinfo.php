...
<?php
	...
	else
	{
		...
		while($row = mysql_fetch_assoc($rst)){
			...
			echo "<tr>";
			echo "<td>{$i}</td>";
			echo "<td><a href='altervolunteerinfo.php?mainid=$mainid&name=$name&term=$term'>{$row['name']}</a></td>";
			echo "<td>{$row['term']}</td>";
...
<?php
	...
	if(!empty($selectaltervolun)){
		...
		if(!empty($selectaltervolunteer)){
			...
			echo "<p>信用 ：<input type='text' name='credit' value='$credit'></p>";
			echo "<p><a href='http://form.mikecrm.com/f.php?t=T4cnX8' target='_blank'>信用规则详情。</a></p>";
...
<?php
	...
	if(!empty($altervolunteerinformation)){
		...
		$sql = "update {$ses} set credit='$credit' where code='$code'";
		$rst = mysql_query($sql) or die("write the wrong value");
		mysql_close();
