...
<?php
	...
	if($num){
		...
		echo "</table>";
		echo "<p><a href='createproject.php'>点此添加/删除项目</a>。</p>";
	}
	else{
		echo "<p>目前没有项目，<a href='createproject.php'>点此处进行创建</a>。</p>";
	}
	echo "<hr/>";
	mysql_close();
	...
	//var_dump($row[0]);
	if($row[0] == "0"){
		echo "<p>目前没有组员，<a href='createmem.php'>点此处进行创建</a>。</p>";	
	}
	else
	{
		...
		if($num = mysql_num_rows($rst)){
			...
			echo "</table>";
			echo "<p><a href='createmem.php'>点此处添加/删除组员</a>。</p>";
		}
		else{
			echo "<p>目前没有组员，<a href='createmem.php'>点此处进行创建</a>。</p>";	
		}
